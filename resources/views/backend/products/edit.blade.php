...
<div style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
    ...
    <div style="flex:1">
        <h2 style="margin:0;font-size:20px;font-weight:700;color:#303030">Edit Product</h2>
        <p style="margin:0;font-size:12px;color:#999">Update product details</p>
    </div>
        ...
        <div style="padding:0 20px 16px">
                ...
                <div>
                    ...
                    <select name="unit_id" class="form-control" required
                        ...
                        @foreach($units as $item)
                            <option value="{{ $item->id }}" {{ old('unit_id', $product->unit_id) == $item->id ? 'selected' : '' }}>{{ $item->title . ' (' . $item->short_name . ')' }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div style="margin-top:16px">
                ...
                <textarea name="description" class="form-control" rows="2" placeholder="Optional product description"
                          style="border-radius:8px;font-size:14px;padding:10px 12px;resize:vertical">{{ old('description', $product->description) }}</textarea>
            </div>
        </div>
    </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;margin-top:12px">
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="purchase_price" class="form-control"
                           placeholder="0.00" value="{{ old('purchase_price', $product->purchase_price) }}" required
                           style="border-radius:8px;font-size:14px;padding:10px 12px">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="discount_type" class="form-control"
                        ...
                        <option value="fixed" {{ old('discount_type', $product->discount_type) == 'fixed' ? 'selected' : '' }}>Fixed (KES)</option>
                ...
                <div>
                    ...
                    <div class="image-upload-container" id="imageUploadContainer">
                        ...
                        <div class="thumb-preview" id="thumbPreviewContainer"
                             style="border:2px dashed #d1d5db;border-radius:10px;padding:16px;text-align:center;cursor:pointer;transition:border-color 0.2s"
                            ...
                            <div class="upload-text d-none" id="uploadText" style="margin-top:8px">
                                <i class="fas fa-cloud-upload-alt" style="font-size:24px;color:#9ca3af"></i>
                                <p style="margin:6px 0 0;font-size:12px;color:#999">Click to change image</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div>
                    ...
                    <div class="input-group date" id="reservationdate" data-target-input="nearest">
                        <input type="text" placeholder="Select expiry date" class="form-control datetimepicker-input"
                               data-target="#reservationdate" name="expire_date" value="{{ old('expire_date', $product->expire_date) }}"
                               style="border-radius:8px;font-size:14px;padding:10px 12px" />
